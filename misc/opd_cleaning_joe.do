...
if ("$sitemap"== "on") & (01 != 0) htput <font size=2pt color="grey"> (Sitemap: Data Management &rarr; import dm opd) </font><br>
htput <H1>$HTtitle</H1>
if ("$mode"!="") htput $mode 
htput <HR> 
 
import delimited using $orig/data/updated/opd_2016-05-26.csv, varnames(1) stringcols(62, 96) clear 
save $orig/data/updated/opd_2016-05-26, replace 

use $orig/data/updated/opd_2016-05-26, clear 

*<<<< VISIT INFORMATION <<<<<<<<

*Health Facility 
replace place="" if place=="NA"
...
drop if yr<2000
	...
	*Correct Date errors according to the launch of each HF 
	replace date=. if (year(date)<2002 & place==2) | (year(date)<2002 & place==3) | (year(date)<2005 & place==4)

	*Create Epidemiological Weeks for OPD 
	epiweek date, epiw(epi_week) epiy(epi_year)
	
*Child referred from another HF
replace referred="" if referred=="NA"
